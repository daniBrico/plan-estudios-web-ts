...
const ThemeToggle = (): JSX.Element => {
  ...
  const handleSelectOp = (value: string): void => {
    console.log(value)
  ...
  return (
    <div className="absolute right-0 mr-4" ref={dropdownRef}>
      ...
      <ul
        className={`bg-theme-first-color absolute z-50 mt-2 flex h-28 w-10 flex-col justify-around rounded-sm border border-solid border-white transition-opacity duration-300 ${isDropdownOpen ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'}`}
      >
        <li
          className="flex items-center justify-center"
          onClick={() => handleSelectOp('ligth')}
        >
